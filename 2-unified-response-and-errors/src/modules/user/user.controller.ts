...
/**
 * HTTP adapter `/users` — minh hoạ success envelope + `BadRequestException` bọc bởi filter (EN: HTTP adapter demonstrating success envelope and filtered validation errors).
 */
@Controller("users")
export class UsersController {
     ...
     *
     * @param body - Payload JSON có thể có `name` (EN: JSON body optionally containing `name`).
     * @returns User vừa tạo (EN: newly created user payload).
     * @sideEffects Có thể ném `BadRequestException` (EN: may throw `BadRequestException`).
     */
    @Post()
    ...
    create(@Body() body: { name?: string }): { id: number; name: string } {
        if (!body?.name?.trim()) {
            throw new BadRequestException(
                "Tên không được để trống (EN: Name is required)",
            )
        }
