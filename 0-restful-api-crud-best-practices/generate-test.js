...
async function testApi() {
    ...
    try {
        ...
        output += '**Luồng 3 — Full update (`PUT /users/:id`)**\n';
        output += 'Trả về ' + r4.status + ':\n```json\n' + JSON.stringify(d4, null, 2) + '\n```\n\n';

        // Luồng 4
        let r5 = await req('/users/' + newId, 'PATCH', { name: "Bob Patched" });
        let d5 = JSON.parse(r5.data);
        output += '**Luồng 4 — Partial update (`PATCH /users/:id`)**\n';
        output += 'Trả về ' + r5.status + ':\n```json\n' + JSON.stringify(d5, null, 2) + '\n```\n\n';

        // Luồng 5
        let r6 = await req('/users/' + newId, 'DELETE');
        output += '**Luồng 5 — Delete (`DELETE /users/:id`)**\n';
        ...
        fs.writeFileSync('c:/Repositories/ac/starci-academy-backend/.mount/data/courses/0-fullstack-mastery/modules/2-rest-api-development-documentation/contents/0-restful-api-crud-best-practices/test.md', output, 'utf8');
